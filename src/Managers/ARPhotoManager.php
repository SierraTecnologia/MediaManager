...
namespace MediaManager\Managers\Photo;

use MediaManager\Models\Photo;
use MediaManager\Services\Image\Contracts\ImageProcessor;
use Locaravel\Managers\LocationManager;
use MediaManager\Managers\PhotoManager;
 ...
 *
 * @package MediaManager\Managers\Photo
 */
        ...
        $this->database->transaction(
            function () use ($photo, $attributes, $thumbnails) {
                if (isset($attributes['location'])) {
                    $photo->localization_id = $this->locationManager->create($attributes['location'])->getId();
                }
                $photo->save();
